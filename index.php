...
function returnCar()
{
	...
	if ($authenticateResult["VALID"] == "true")
	{
		...
		if (isCarReturned($q->bookingNo) == false)
		{
			...
			// function in db_functions.php return boolean
...
{		
	...
	$row = retrieveBrands();  // function in db_functions.php return rows or null
...
{		
	...
	$row = retrieveAvailableCars($a_suburb, $a_pickupDate, $a_dropoffDate);  // function in db_functions.php
...
{		
	...
	$row = retrieveSuburbs();  // function in db_functions.php
...
{		
	...
	if ($authenticateResult["VALID"] == "true")
	{
		// echo "inside auth key ok";
		// update row
		$row = retrieveCustomer($an_email);  // function in db_functions.php
	}
...
{		
	...
	if ($authenticateResult["VALID"] == "true")
	{
		...
		$row = retrieveBookings($a_customerId);  // function in db_functions.php
	}	
...
{		
	...
	if ($authenticateResult["VALID"] == "true")
	{
		...
		// get bookings
		// function in db_functions.php
		$row = retrieveUnreturnedBookings($a_customerId);  // function in db_functions.php
	}	
...
function areDatesColliding($customerId, $startDate, $returnDate)
{
	...
	// function in db_functions.php return rows [{},{}]
...
function isTheEmailUnique($anEmail)
{
	...
	// function in db_functions.php return rows (zero or one)
